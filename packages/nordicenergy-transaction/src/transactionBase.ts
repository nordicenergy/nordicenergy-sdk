...
import { BN, getAddress, NordicEnergyAddress } from '@nordicenergy-js/crypto';
...
export class TransactionBase implements AbstractTransaction {
  static normalizeAddress(address: string) {
    if (address === '0x') {
      ...
      NordicEnergyAddress.isValidChecksum(address) ||
      NordicEnergyAddress.isValidBech32(address) ||
      NordicEnergyAddress.isValidBech32TestNet(address)
    ) {
