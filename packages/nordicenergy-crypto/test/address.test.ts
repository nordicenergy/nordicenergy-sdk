...
import { getAddress, NordicEnergyAddress } from '../src/address';
...
describe('test address', () => {
  it('should test NordicEnergyAddress', () => {
    const hAddress = new NordicEnergyAddress(bytes);
    ...
    expect(NordicEnergyAddress.isValidBasic(bytes)).toEqual(true);
    expect(NordicEnergyAddress.isValidChecksum(checksum)).toEqual(true);
    expect(NordicEnergyAddress.isValidBech32(bech32)).toEqual(true);
    expect(NordicEnergyAddress.isValidBech32TestNet(bech32Testnet)).toEqual(true);
