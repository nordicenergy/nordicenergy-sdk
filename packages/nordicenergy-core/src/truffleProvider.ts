...
import { ChainID, ChainType, Unit, isBech32Address } from '@nordicenergy-js/utils';
import { HDNode } from '@nordicenergy-js/account';
import { fromBech32, HRP } from '@nordicenergy-js/crypto';
...
export class TruffleProvider extends HDNode {
  ...
  async send(...args: [RPCRequestPayload<any>, any]) {
    ...
    switch (newMethod) {
      ...
      case 'net_getTransactionReceipt': {
        const result = await this.provider.send(
          {
            id,
            method: 'net_getTransactionReceipt',
            params: [params[0]],
            jsonrpc: '2.0',
          },
          (err: any, res: any) => {
      ...
      case 'web3_clientVersion': {
        callback(null, {
          result: `NordicEnergy/${packageInfo.version}/@nordicenergy-js`,
          id,
          jsonrpc: '2.0',
        });
        return {
  ...
  resolveArgs(...args: [RPCRequestPayload<any>, any]): ArgsResolver {
    ...
    if (method.startsWith('eth')) {
      newMethod = method.replace('eth', 'net');
    }
