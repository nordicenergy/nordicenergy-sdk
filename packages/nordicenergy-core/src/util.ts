...
import { ChainType, ChainID, defaultConfig } from '@nordicenergy-js/utils';
import { NordicEnergy } from './nordicenergy';

export interface nordicenergyConfig {
  chainUrl?: string;
  chainType: ChainType;
  chainId: ChainID;
  shardID?: number;
...
export function createWeb3(_web3: any) {
  ...
  const nordicenergy = new NordicEnergy(url, {
    chainId: defaultConfig.Default.Chain_ID,
    chainType: defaultConfig.Default.Chain_Type,
    chainUrl: defaultConfig.Default.Chain_URL,
  });

  _web3.setProvider(nordicenergy.messenger.provider);
  _web3.messenger = nordicenergy.messenger;
  _web3.eth.getRpcResult = nordicenergy.blockchain.getRpcResult;

  // map blockchain to eth
  const { blockchain } = nordicenergy;
  ...
  _web3.eth.subscribe = nordicenergy.messenger.subscribe;

  // map accounts to _web3
  _web3.accounts = nordicenergy.wallet.accounts;
  _web3.eth.accounts.create = nordicenergy.wallet.createAccount;
  _web3.eth.accounts.privateKeyToAccount = nordicenergy.wallet.addByPrivateKey;
  _web3.eth.accounts.encrypt = async (privateKey: string, password: string) => {
    const newAcc = new nordicenergy.Modules.Account(privateKey, nordicenergy.messenger);
    const result = await newAcc.toFile(password);
    return result;
  };

  _web3.eth.accounts.decrypt = async (keystoreJsonV3: any, password: string) => {
    const newAcc = new nordicenergy.Modules.Account();
    const result = await newAcc.fromFile(JSON.stringify(keystoreJsonV3), password);
    return result;
  };

  _web3.eth.accounts.signTransaction = nordicenergy.wallet.signTransaction;

  // map transaction to web3
  _web3.eth.recoverTransaction = nordicenergy.transactions.recover;

  // map contract to web3
  _web3.eth.Contract = nordicenergy.contracts.createContract;

  _web3.utils = { ..._web3.utils, ...nordicenergy.utils, ...nordicenergy.crypto };
}
