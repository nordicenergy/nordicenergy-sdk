...
This package provides a collection of apis to interact with PowerChain blockchain.
...
import {
  ...
  // NordicEnergyCore,
  DefaultBlockParams,
} from '@nordicenergy-js/utils';

import { getAddress } from '@nordicenergy-js/crypto';

import { Transaction } from '@nordicenergy-js/transaction';
import { StakingTransaction } from '@nordicenergy-js/staking';
...
class Blockchain {
   ...
   * @example
   * ```javascript
   * net.blockchain.getBalance({
   *   address: 'net103q7qe5t2505lypvltkqtddaef5tzfxwsse4z7',
   *   blockNumber: 'latest'
   * }).then(value => {
   *   console.log(value.result);
   * });
   ...
   * @example
   * ```javascript
   * net.blockchain.getBlockNumber().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * net.blockchain.getBlockByNumber({
   *   blockNumber: '0x89',
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * @example
   * ```javascript
   * net.blockchain.getBlockTransactionCountByHash({
   ...
   * @example
   * ```javascript
   * net.blockchain.getBlockTransactionCountByNumber({
   *   blockNumber: '0x2403C'
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * @example
   * ```javascript
   * net.blockchain.getTransactionByBlockHashAndIndex({
   *   blockHash: '0x4142514a238157e7fe57b9d54abedb33943507fa15b3799954c273a12705ced1',
   *   index: '0x0'
   * }).then((value) => {
   *   console.log(value);
   ...
   * @example
   * ```javascript
   * net.blockchain.getTransactionByBlockNumberAndIndex({
   *   blockNumber: '0x2403C',
   *   index: '0x0'
   * }).then((value) => {
   *   console.log(value);
   ...
   * @example
   * ```javascript
   * net.blockchain.getTransactionByHash({
   ...
   * @example
   * ```javascript
   * net.blockchain.getTransactionReceipt({
   *   txnHash: '0x146a0cf7e8da45b44194207c4e7785564527059483b765f9a04424554443b224'
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * net.blockchain.getCxReceiptByHash({
   ...
   * @example
   * ```javascript
   * net.blockchain.net_peerCount().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * net.blockchain.net_version().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * net.blockchain.getProtocolVersion().then((value) => {
   *   console.log(value.result);
   * });
   * ```
   */
   ...
   * @example
   * ```javascript
   * net.blockchain.getStorageAt({
   *   address: 'net1d0kw95t6kkljmkk9vu0zv25jraut8ngv5vrs5g',
   *   position: '0x0'
   * }).then((value) => {
   *   console.log(value);
   * });
   ...
   * @example
   * ```javascript
   * net.blockchain.getTransactionCount({
   *   address: "net1d0kw95t6kkljmkk9vu0zv25jraut8ngv5vrs5g"
   * }).then((value) => {
   *   console.log(value.result);
   * });
   ...
   * @example
   * ```javascript
   * net.blockchain.getShardingStructure().then((value) => {
   *   console.log(value);
   * });
   * ```
   */
   ...
   * net.wallet.addByPrivateKey(privateKey);
   *
   * async function transfer() {
   ...
   *     toShardID: 0,
   *     // gas Price, you can use Unit class, and use Gwei, then remember to use toWei(), which will be transformed to BN
   *     gasPrice: new net.utils.Unit('100').asGwei().toWei(),
   *   });
   *
   *   // sign the transaction use wallet;
   *   const signedTxn = await net.wallet.signTransaction(txn);
   *   const txnHash = await net.blockchain.sendTransaction(signedTxn);
   *   console.log(txnHash.result);
   * }
   *
   * transfer();
   ...
   *
   *   // sign the transaction use wallet;
   *   const signedTxn = await ndg.wallet.signTransaction(txn);
   *   recovered = signedTxn.recover(signedTxn.rawTransaction);
   *
   *   const txnHash = await net.blockchain.sendRawTransaction(recovered);
   *   console.log(txnHash);
   * }
   *
   * transfer();
   ...
   *   // sign the transaction use wallet;
   *   const signedTxn = await net.wallet.signTransaction(txn);
   *   const txnHash = await net.blockchain.createObservedTransaction(signedTxn);
   *   console.log(txnHash);
   * }
   *
   * transfer();
   ...
   * @example
   * ```javascript
   * net.blockchain.estimateGas({
   *   to: 'net1d0kw95t6kkljmkk9vu0zv25jraut8ngv5vrs5g',
   ...
   * @example
   * ```javascript
   * net.blockchain.gasPrice().then((value) => {
   *   console.log(value);
   * });
   * ```
   */
   ...
   * );
   *
   * const tmp = net.blockchain.newPendingTransactions();
   * console.log(tmp)
   * ```
   */
   ...
   * );
   *
   * const tmp = net.blockchain.newBlockHeaders();
   * console.log(tmp)
   * ```
   */
   ...
   * );
   *
   * const tmp = net.blockchain.syncing();
   * console.log(tmp)
   * ```
   */
