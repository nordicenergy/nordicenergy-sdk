...
import { NordicEnergyCore, ChainType, isString, ChainID, defaultConfig } from '@nordicenergy-js/utils';
 ...
 * ```
 */
class Messenger extends NordicEnergyCore {
  ...
  async setShardingProviders() {
    if (this.chainPrefix !== ChainType.NordicEnergy) {
      return;
    }
    try {
