...
import { bip39, hdkey, getAddress, BN, Signature } from '@nordicenergy-js/crypto';
import {
  HDPath,
  // defineReadOnly,
  isHttp,
...
import { Messenger, HttpProvider, WSProvider, RPCMethod } from '@nordicenergy-js/network';
import { Transaction, TxStatus, TransasctionReceipt } from '@nordicenergy-js/transaction';
...
export class HDNode {
  ...
  constructor(
    ...
    shardID: number = 0,
    chainType: ChainType = ChainType.NordicEnergy,
    ...
    this.addresses = [];
    this.wallets = {};
    this.path = chainType === ChainType.NordicEnergy ? HDPath : `m/44'/60'/0'/0/`;
    this.index = index;
